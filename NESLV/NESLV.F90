recursive subroutine neslv(x,fp,gp,func,fjac,typx,typy,&
	...
	do while(termcode==0)
		...
		if(analjac) then
			...
			if(restart .or. method==1) then
				fvec(:)=func(x)
				nfnt=nfnt+1
				call nefdjac(x,func,fvec,jc,sx) !diff jac
				nfnt=nfnt+n
				njnt=njnt+1
				call unit_matrix(qt(:,:))
				call put_diag(sy(:),qt(:,:))
		...
		case(4)
			...
			call nefdjac(xold,func,fvcold,jc,sx) !计算JACOBIAN
